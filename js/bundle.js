...
function refresh(state) {
  ...
  const showSetting = !!state.inBattle;
  ...
  if (btnAutoBattleSettingRepeat) {
    const canRepeat = hasAdventureFarmingTarget();
    btnAutoBattleSettingRepeat.textContent = autoRetryLabel;
...
function finalizeBattle(reason){
  ...
  state.autoBattleSettingsOpen = false;
  clearStatuses(state.enemy);
  state.enemy = null;
  state.playerDefending = false;
  clearStatuses(state.player);
...
function bind() {
  const setClick = (id, handler) => {
    const list = Array.from(document.querySelectorAll(`[id="${id}"]`));
    if (!list.length) {
      console.warn(`[BIND] elemen #${id} tidak ditemukan, skip binding.`);
      return;
    }
    list.forEach((el) => {
      el.onclick = handler;
  ...
  if (btnAutoBattleSettingsFloating) {
    btnAutoBattleSettingsFloating.onclick = () => {
      ...
      state.autoBattleSettingsOpen = !state.autoBattleSettingsOpen;
      refresh(state);
    };
  }
...
function loadLocalAndShowCharacterMenu(msg = "Mode offline aktif. Pilih karakter."){
  const local = load();
  applyLoaded(local || emptyProfilePayload());
  showAuth(false);
  openCharacterMenu(msg);
}
...
(function boot() {
  ...
  bindAuthClick("authLogin", () => doLogin());
  bindAuthClick("authRegister", () => doRegister());
  bindAuthClick("authOffline", () => {
    offlineModeChosen = true;
    setAuthMsg("Mode offline aktif.", false);
    loadLocalAndShowCharacterMenu();
  });
